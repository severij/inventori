...
import type { BreadcrumbItem, EntityType } from '../types';
...
export function useAncestors(
  ...
  const refetch = useCallback(async () => {
    ...
    try {
      ...
      let currentType: EntityType | undefined = type;
      ...
      while (currentId && currentType) {
        if (currentType === 'location') {
          ...
          const container = await getContainer(currentId);
          if (container) {
            path.unshift({ id: container.id, name: container.name, type: 'container' });
            currentId = container.parentId;
            currentType = container.parentType;
          } else {
            break;
          }
